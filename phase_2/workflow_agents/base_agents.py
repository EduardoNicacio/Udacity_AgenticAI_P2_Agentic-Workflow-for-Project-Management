...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):
            ...
            )  #  As per todo 3 - Obtain a response from the worker agent
            print(f"Worker Agent Response:\n{response_from_worker}")

            print(" Step 2: Evaluator agent judges the response")
            eval_prompt = (
                f"Does the following answer: {response_from_worker}\n"
                f"Meet this criteria: {self.evaluation_criteria}"  #  As per todo 4 - Insert evaluation criteria here
                f"Respond Yes or No, and the reason why it does or doesn't meet the criteria."
            )
            response = client.chat.completions.create(
                model=OPENAI_BASE_MODEL,
                #  As per todo 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model=OPENAI_BASE_MODEL,
                    #  As per todo 6 - Define the message structure sent to the LLM to generate correction instructions (use temperature=0)
        ...
        return {
            #  As per todo 7 - Return a dictionary containing the final response, evaluation, and number of iterations
...
class RoutingAgent:
    ...
    #  As per todo 3 - Define a method to route user prompts to the appropriate agent
    def route(self, user_input):
        ...
        """
        #  As per todo 4 - Compute the embedding of the user input prompt
        input_emb = self.get_embedding(user_input)
        best_agent = None
        best_score = -1

        for agent in self.agents:
            #  As per todo 5 - Compute the embedding of the agent description
            ...
            #  As per todo 6 - Add logic to select the best agent based on the similarity score between the user prompt and the agent descriptions
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        #  As per todo 4 - Extract the response text from the OpenAI API response
        response_text = response.choices[0].message.content

        #  As per todo 5 - Clean and format the extracted steps by removing empty lines
